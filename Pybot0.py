...
if 'arm' in sys.platform:# == 'linux-armv71': 
	...
	channel = 'droid.recognizeSpeech("'+prompt+'",None,None).result'
...
def main(*args):
	response='hi';input='search hi';choice=None;YorN=None; words = ['']
	#### MAIN LOOP:
	global droid
	while response is not None:
		...
		#### text input
		if not response: exec(channel)
		if choice: prompt = choice; exec(channel)
		if not choice and response: prompt = response+'>'; exec(channel)
		#exec(channel)
		#exec('print 2')
		# if input is None: 
			# prompt = response+'>'
		...
		if os.name == 'posix':#sys.platform == 'linux-armv71':
			print 'speaking'
